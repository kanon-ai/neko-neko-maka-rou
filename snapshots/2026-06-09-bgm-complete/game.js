(() => {
  ...
  function setupFloor(keepStats = false) {
    ...
    const old = player;
    player = {
      x: start.x, y: start.y, hp: keepStats ? old.hp : 9, maxHp: keepStats ? old.maxHp : 9,
    ...
    for (let i = 0; i < 4 + floor * 2; i++) {
      ...
      enemies.push({ ...p, type, hp: stats[0], maxHp: stats[0], attack: stats[1], awake: false });
    }
    computeFov();
    updateUi();
  ...
  function moveEnemy(enemy) {
    ...
    if (distance(enemy, player) === 1) {
      ...
      addLog(`${enemyName(enemy)}の攻撃。-${enemy.attack} HP`);
      return;
    }
